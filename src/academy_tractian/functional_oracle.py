...
class FunctionalCaseSpec(_FrozenModel):
    ...
    forbidden_reason_codes: tuple[str, ...] = (
        "DECISION_SOURCE_FAILURE", "DECISION_SOURCE_AUDIT_FAILURE", "TOOL_BOUNDARY_FAILURE",
        "TOOL_CALL_BUDGET_EXHAUSTED", "TURN_BUDGET_EXHAUSTED",
    )
...
class FunctionalOracleReport(_FrozenModel):
    ...
    def as_independent_oracle(self) -> IndependentOracleResult:
        return IndependentOracleResult(
            ...
            summary=(f"Functional case {self.case_id} satisfied its independent rubric." if self.status == "VERIFIED" else f"Functional case {self.case_id} failed: {', '.join(self.failures)}."),
...
class EvidenceOracleReport(_FrozenModel):
    ...
    def as_independent_oracle(self) -> IndependentOracleResult:
        return IndependentOracleResult(
            status=self.status,
            source=self.schema_version,
            summary=(f"Evidence requirements {self.requirement_id} were satisfied." if self.status == "VERIFIED" else f"Evidence requirements {self.requirement_id} failed: {', '.join(self.failures)}."),
            evidence=tuple(self.failures) if self.failures else (f"requirements:{self.requirement_id}",),
            metrics={"satisfied_groups": self.satisfied_groups, "required_groups": self.required_groups, "failure_count": len(self.failures)},
        )
...
def _successful_calls(trace: RunTrace) -> list[TraceEvent]:
    ...
    return [event for event in trace.events if event.event_type == "tool_call" and event.call_id is not None and event.call_id in successful_ids]
...
def _resource_observed(calls: list[TraceEvent], requirement: ToolResourceRequirement) -> bool:
    return any(call.tool_name in requirement.tools and _contains_scalar(call.arguments or {}, requirement.resource_id) for call in calls)
...
def evaluate_functional_trace(trace: RunTrace, spec: FunctionalCaseSpec) -> FunctionalOracleReport:
    ...
    if spec.allowed_response_modes and response_mode not in {mode.value for mode in spec.allowed_response_modes}:
    ...
    return FunctionalOracleReport(
        case_id=spec.case_id, status="FAILED" if failures else "VERIFIED", failures=tuple(failures),
...
def evaluate_evidence_trace(trace: RunTrace, spec: EvidenceRequirementSpec) -> EvidenceOracleReport:
    ...
    return EvidenceOracleReport(
        requirement_id=spec.requirement_id, status="FAILED" if failures else "VERIFIED",
        failures=tuple(failures), satisfied_groups=satisfied_groups,
