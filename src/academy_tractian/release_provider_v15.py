...
from .release_provider_v14 import RELEASE0_V14_SYSTEM_INSTRUCTION


NVIDIA_PROVIDER_ID = "nvidia"
NVIDIA_MODEL_ID = "openai/gpt-oss-20b"
NVIDIA_ROUTE_ID = "nvidia.hosted_nim.chat_completions.v1.gpt_oss_20b"
NVIDIA_ENDPOINT = "https://integrate.api.nvidia.com/v1/chat/completions"
NVIDIA_ACCOUNT_SENTINEL = "nvidia"
RELEASE0_V15_PROVIDER_VERSION = "release0-provider-nvidia-v15"

# V14 already includes the V13 grounding instruction. Keep this alias explicit so a future
# instruction change cannot accidentally make V15 depend on a generic runtime path.
RELEASE0_V15_SYSTEM_INSTRUCTION = RELEASE0_V14_SYSTEM_INSTRUCTION
assert RELEASE0_V13_GROUNDING_INSTRUCTION in RELEASE0_V15_SYSTEM_INSTRUCTION


class NvidiaV15ServingError(RuntimeError):
    """Sanitized NVIDIA serving failure; never includes credentials, prompts, tools or body text."""

    def __init__(self, *, status_code: int | None, category: str) -> None:
        self.status_code = status_code
        self.category = category
        status = "none" if status_code is None else str(status_code)
        super().__init__(
            f"nvidia_v15_serving_failure:{category}:status={status}:"
            f"model={NVIDIA_MODEL_ID}:endpoint_host=integrate.api.nvidia.com"
        )
...
def _accept_completion_content(*, finish_reason: Any, content: Any) -> str:
...
class Release0NvidiaDecisionClientV15:
    """One-shot NVIDIA hosted-NIM client for the V15 composition path only.

    The serving model is the tested active sibling of the deprecated historical 120B route.
    There is no retry, automatic model/provider fallback, output repair, or credential lookup.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(api_key, str) or not api_key.strip():
            raise ValueError("NVIDIA V15 client requires an explicit non-empty api_key")
    ...
    def build_http_request(self, request: ProviderDecisionRequest) -> ProviderHttpRequest:
        body: dict[str, Any] = {
            "model": self.model_id,
            "messages": [
                {"role": "system", "content": RELEASE0_V15_SYSTEM_INSTRUCTION},
    ...
    def _invoke_once(self, request: ProviderHttpRequest) -> Mapping[str, Any]:
        try:
            ...
            if exc.status_code in {404, 410}:
                raise NvidiaV15ServingError(
                    status_code=exc.status_code,
                    category="MODEL_OR_ROUTE_UNAVAILABLE",
                ) from None
            raise
        except Exception:
    ...
    def complete(self, request: ProviderDecisionRequest) -> str:
        ...
        if message.get("tool_calls") not in (None, []):
            # Production Release 0 uses application-owned JSON decisions. Native tool_calls were
            # qualified separately by the serving probe and are never executed by this client.
        ...
        content = _accept_completion_content(
