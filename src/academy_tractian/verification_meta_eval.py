...
class MutationOutcome(_FrozenModel):
    """One labelled evaluator mutation trial.

    `defect_present` is the independent label. `evaluator_failed` is the evaluator's observed
    classification. The evaluator under test must not generate its own ground-truth label.
    """
...
def score_mutation_outcomes(
    outcomes: list[MutationOutcome] | tuple[MutationOutcome, ...],
    *,
    minimum_sensitivity: float = 0.95,
    minimum_specificity: float = 0.90,
) -> MetaEvaluationReport:
    """Score an evaluator against independent mutation labels.

    Promotion is fail-closed: any critical false negative blocks promotion even if aggregate
    metrics remain above threshold. Empty mutation suites are never promotable.
    """
    ...
    critical_fn = tuple(
        ...
        if item.critical and item.defect_present and not item.evaluator_failed
    )

    has_positive_labels = any(item.defect_present for item in outcomes)
    has_negative_labels = any(not item.defect_present for item in outcomes)
    promotable = (
        bool(outcomes)
        and has_positive_labels
        and has_negative_labels
        and not critical_fn
