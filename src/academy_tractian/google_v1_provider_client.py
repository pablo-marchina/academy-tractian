...
GOOGLE_V1_PROVIDER_ID = "google"
GOOGLE_37_MODEL_ID = "gemini-3.7-flash"
GOOGLE_38_MODEL_ID = "gemini-3.8-flash"
GOOGLE_V1_SUPPORTED_MODELS = frozenset({GOOGLE_37_MODEL_ID, GOOGLE_38_MODEL_ID})
GOOGLE_V1_ROUTE_ID = "google.interactions.v1.stateless"
GOOGLE_V1_INTERACTIONS_ENDPOINT = "https://generativelanguage.googleapis.com/v1/interactions"
...
class GoogleV1InteractionsDecisionClient:
    """One-shot stable-v1 Gemini Interactions client for controlled hosted candidates.

    Model choice is explicit rather than provider-implied so Gemini versions can be compared under
    identical runtime/evaluator conditions. Server-side conversation state, tools and thought
    summaries are disabled; application-owned tools and safety boundaries remain authoritative.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if normalized_model not in GOOGLE_V1_SUPPORTED_MODELS:
            raise ValueError("unsupported_google_v1_model")
    ...
    def build_http_request(self, request: ProviderDecisionRequest) -> ProviderHttpRequest:
        ...
        return ProviderHttpRequest(
            method="POST",
            url=GOOGLE_V1_INTERACTIONS_ENDPOINT,
            headers={
    ...
    def complete(self, request: ProviderDecisionRequest) -> str:
        ...
        return _extract_google_v1_output(response.body, expected_model=self.model_id)
...
def _extract_google_v1_output(response: Mapping[str, Any], *, expected_model: str) -> str:
    ...
    for step in steps:
        ...
        if step_type in {"thought", "user_input"}:
            continue
