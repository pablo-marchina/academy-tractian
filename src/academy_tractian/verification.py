...
def _availability_dimension(run: Mapping[str, Any], events: Sequence[Mapping[str, Any]]) -> VerificationDimension:
...
def _trajectory_dimension(run: Mapping[str, Any], events: Sequence[Mapping[str, Any]]) -> VerificationDimension:
    reason = _failure_reason(run)
    ...
    for event in events:
        ...
        if status_code is None or status_code < 400:
            continue
        key = (str(event.get("tool_name") or ""), str(event.get("path_template") or ""), status_code)
...
def _oracle_dimension(*, name: str, oracle: IndependentOracleResult | None, default_failure: str | None = None, blocking: bool) -> VerificationDimension:
...
def verify_persisted_run(*, run: Mapping[str, Any], events: Sequence[Mapping[str, Any]], evaluation: Sequence[Mapping[str, Any]], oracle_results: Mapping[str, IndependentOracleResult] | None = None) -> RunVerificationReport:
    ...
    dimensions = (
        ...
        _oracle_dimension(
            ...
            default_failure=(f"Functional success is impossible for this run because it terminated with {hard_runtime_failure}." if hard_runtime_failure is not None else None),
            blocking=True,
        ),
        _oracle_dimension(
            name="evidence_sufficiency",
            oracle=oracle_results.get("evidence_sufficiency"),
            default_failure=(f"Evidence sufficiency was not achieved because the run terminated with {hard_runtime_failure}." if hard_runtime_failure is not None else None),
            blocking=True,
        ),
        _trajectory_dimension(run, events),
        _availability_dimension(run, events),
        _safety_dimension(evaluation),
        _oracle_dimension(name="semantic_correctness", oracle=oracle_results.get("semantic_correctness"), blocking=False),
        _oracle_dimension(name="operational_value", oracle=oracle_results.get("operational_value"), blocking=False),
    )
    ...
    return RunVerificationReport(run_id=run_id, overall_status=overall, dimensions=dimensions)
