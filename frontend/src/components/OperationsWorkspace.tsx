...
export type OperationsWorkspaceFocus = "analysis" | "quality" | "data" | "system";
...
export function OperationsWorkspace({
  ...
  focus?: OperationsWorkspaceFocus;
  onFocusChange?: (focus: OperationsWorkspaceFocus) => void;
}) {
  ...
  const overviewQuery = useQuery({
    ...
    refetchInterval: focus === "system" ? 3_000 : false,
    enabled: focus === "system",
  });
  const healthQuery = useQuery({
    queryKey: ["production-health"],
    queryFn: fetchProductionHealth,
    refetchInterval: focus === "system" ? 2_000 : false,
    enabled: focus === "system",
  });
  const toolsQuery = useQuery({
    queryKey: ["tools-metrics", selectedRunId],
    queryFn: () => fetchToolsMetrics(selectedRunId),
    refetchInterval: focus === "analysis" ? 5_000 : false,
    enabled: focus === "analysis",
  });
  const policiesQuery = useQuery({
    queryKey: ["policies-metrics", selectedRunId],
    queryFn: () => fetchPoliciesMetrics(selectedRunId),
    refetchInterval: focus === "analysis" ? 5_000 : false,
    enabled: focus === "analysis",
  });
  const evaluationMetricsQuery = useQuery({
    queryKey: ["evaluation-metrics", selectedRunId],
    queryFn: () => fetchEvaluationMetrics(selectedRunId),
    refetchInterval: focus === "quality" ? 5_000 : false,
    enabled: focus === "quality",
  });
  const providerQuery = useQuery({
    queryKey: ["provider-experiments"],
    queryFn: fetchProviderExperiments,
    staleTime: 60_000,
    enabled: focus === "quality",
  });
  const evidenceQuery = useQuery({
    ...
    enabled: focus === "analysis" && Boolean(selectedRunId),
    refetchInterval: focus === "analysis" && selectedRunId ? 1_500 : false,
  });
  ...
  if (focus === "data") {
    return (
      <section className="operations-workspace" aria-label="Data exploration">
  ...
  if (focus === "system") {
    return (
      <section className="operations-workspace" aria-label="Production system state">
        <article className="panel operations-panel mission-control">
          <div className="section-heading compact">
            <div><h2>Production health</h2><p className="section-supporting-copy">Live service state, runtime pressure and measured delivery health.</p></div>
          ...
          {health ? (
            <>
              ...
              <div className="instrumentation-gap"><strong>Still not measured</strong><span>{health.not_measured_yet.length ? health.not_measured_yet.join(" · ") : "none"}</span></div>
            </>
  ...
  if (focus === "analysis") {
    return (
      <section className="operations-workspace" aria-label="Current analysis internals">
        <div className="analytics-scope-banner global-scope">
          <strong>Analysis scope</strong>
          <span title={selectedRunId ?? undefined}>{selectedRunId ?? "all persisted runs"}</span>
          <small>Evidence, lineage, tools and policy use this scope.</small>
        </div>

        <div className="operations-two-column">
          <article className="panel operations-panel">
            <div className="section-heading compact"><div><h2>Evidence references</h2></div><span className="count-pill">{evidenceQuery.data?.count ?? 0} refs</span></div>
            {!selectedRunId ? <div className="empty-state small"><strong>No analysis selected</strong><p>Choose an analysis before inspecting persisted evidence references.</p></div>
              ...
              ) : <div className="empty-state small"><strong>No safe evidence refs</strong><p>This analysis has not emitted a persisted evidence reference.</p></div>}
          </article>

          <article className="panel operations-panel lineage-panel">
            <div className="section-heading compact"><div><h2>Output lineage</h2></div><span className="count-pill">{lineageQuery.data?.cards.length ?? 0} rows</span></div>
            {!selectedRunId ? <div className="empty-state small"><strong>No analysis selected</strong><p>Lineage is never synthesized without a real analysis.</p></div>
              : lineageQuery.data?.cards.length ? (
                <ol className="lineage-list">{lineageQuery.data.cards.map((card) => (
                  <li key={card.lineage_id} className={card.origin === "EVALUATOR" ? "lineage-evaluator" : ""}>
                    ...
                    <div>
                      <div className="lineage-heading"><span className="origin-badge">{card.origin}</span><strong>{card.event_type}</strong></div>
              ...
              ) : <div className="empty-state small"><strong>No lineage rows</strong><p>The backend has no safe events for this analysis yet.</p></div>}
          </article>
        </div>

        <div className="operations-two-column">
          <article className="panel operations-panel">
            <div className="section-heading compact"><div><h2>Tool activity</h2></div><span className="count-pill">{toolsQuery.data?.count ?? 0} tools</span></div>
            {toolOption && toolsQuery.data?.items.length ? <><EChart option={toolOption} onDataPointClick={toolDrilldown} /><div className="compact-data-list">{toolsQuery.data.items.map((item) => <div key={item.tool_name}><div><strong>{item.tool_name}</strong><button className="drilldown-button" type="button" onClick={() => requestDrilldown({ dataset: "events", dimension: "event_type", filterField: "tool_name", filterValue: item.tool_name, chartType: "bar" })}>Explore data</button></div><span>{item.calls} calls · {item.results} results · {item.observations} observations</span></div>)}</div></> : <p className="muted">No persisted tool activity in this scope.</p>}
          </article>
          <article className="panel operations-panel">
            <div className="section-heading compact"><div><h2>Policy checks</h2></div><span className="count-pill">{policiesQuery.data?.count ?? 0} stages</span></div>
            {policyOption && policiesQuery.data?.items.length ? <><EChart option={policyOption} onDataPointClick={policyDrilldown} /><div className="compact-data-list">{policiesQuery.data.items.map((item) => <div key={item.policy_stage}><div><strong>{item.policy_stage}</strong><button className="drilldown-button" type="button" onClick={() => requestDrilldown({ dataset: "events", dimension: "policy_allowed", filterField: "policy_stage", filterValue: item.policy_stage, chartType: "bar" })}>Explore data</button></div><span>{item.checks} checks · {percent(item.block_rate)} blocked · {item.contained} contained</span></div>)}</div></> : <p className="muted">No persisted policy checks in this scope.</p>}
          </article>
        </div>
      </section>
    );
  ...
  return (
    <section className="operations-workspace" aria-label="Evaluation and provider quality">
      <div className="operations-two-column">
        <article className="panel operations-panel">
          <div className="section-heading compact"><div><h2>Evaluation metrics</h2></div>{evaluationMetricsQuery.data && <span className="count-pill">blocking {percent(evaluationMetricsQuery.data.blocking_pass_rate)}</span>}</div>
          {evalOption && evaluationMetricsQuery.data?.checks.length ? <><EChart option={evalOption} height={320} onDataPointClick={evaluationDrilldown} /><div className="compact-data-list">{evaluationMetricsQuery.data.checks.map((item) => <div key={item.check_name}><div><strong>{item.check_name}</strong><button className="drilldown-button" type="button" onClick={() => requestDrilldown({ dataset: "evaluations", dimension: "passed", filterField: "check_name", filterValue: item.check_name, chartType: "bar" })}>Explore data</button></div><span>{percent(item.pass_rate)} pass · {item.evaluations} evaluations</span></div>)}</div><div className="eval-summary"><span>overall pass {percent(evaluationMetricsQuery.data.overall_pass_rate)}</span><span>{evaluationMetricsQuery.data.rows} check rows</span></div></> : <p className="muted">No persisted evaluation aggregate in this scope.</p>}
        </article>

        <article className="panel operations-panel provider-lab">
          <div className="section-heading compact"><div><h2>Provider experiments</h2></div>{providerQuery.data && <span className="count-pill" title={providerQuery.data.registry_sha256}>registry {providerQuery.data.registry_sha256.slice(0, 10)}</span>}</div>
            ...
            <div className="experiment-card">
              ...
              {d01 && d01.packet_observed_neurons !== null && d02.packet_observed_neurons !== null && (
                <div className="diagnostic-note">
                  ...
                  <div className="compact-data-list">{providerDeltas.map(({ current, baseline }) => <div key={current.candidate_id}><strong>{current.candidate_id}</strong><span>success {percentagePoints(current.success_rate - baseline.success_rate)} · M1 {percentagePoints(current.structured_decision_adherence - baseline.structured_decision_adherence)} · M4 {percentagePoints(current.public_task_quality - baseline.public_task_quality)} · M7 {percentagePoints(current.signature_stability - baseline.signature_stability)} · median latency {relativeDelta(current.median_latency_ms, baseline.median_latency_ms)} · Neurons {relativeDelta(current.observed_neurons, baseline.observed_neurons)}</span></div>)}</div>
