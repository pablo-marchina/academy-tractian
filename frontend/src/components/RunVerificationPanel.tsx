      ...
      <div className="section-heading compact">
        <div>
          ...
          <p className="section-supporting-copy">
            Runtime integrity, task success, evidence, trajectory, availability and safety are separate claims. A structural green trace is never presented as overall quality.
          </p>
        </div>
      </div>
      ...
      {!runId ? (
        ...
        <>
          <div className="evaluation-score">
            <strong>{query.data.overall_status}</strong>
            <span>overall hard-gate status</span>
          </div>
          <ul className="check-list">
            {query.data.dimensions.map((dimension) => <DimensionRow key={dimension.name} dimension={dimension} />)}
          </ul>
