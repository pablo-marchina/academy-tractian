#!/usr/bin/env python3
"""E10g full DEV+VALIDATION fixed capture runner.

This runner remeasures the E10g balanced safety-action guard after the
DEV-only safety/action gate passed. It runs DEV and VALIDATION as measurement
splits only, keeps LOCKED_TEST blocked, and never gives private expected paths,
evaluator labels, validation feedback, or LOCKED_TEST material to the model or
guard.
...
SPEC_G = importlib.util.spec_from_file_location("e10g_guard", E10G_PATH)
if SPEC_G is None or SPEC_G.loader is None:
    raise RuntimeError("failed to load e10g_dev_only_balanced_safety_action_guard.py")
e10g = importlib.util.module_from_spec(SPEC_G)
SPEC_G.loader.exec_module(e10g)
base = e10g.base
...
def apply_e10g_guard_to_stage(stage: dict[str, Any]) -> dict[str, Any]:
    ...
    for call in calls:
        ...
        if not isinstance(output, dict):
            continue
        guarded, guard_meta = e10g.apply_balanced_guard_to_output(output)
        call["parsed_output"] = guarded
        call["output_hash"] = base.stable_hash(guarded)
        call["score"] = e10g.e10e.e10d.e10b.capture.score_output(guarded, json.dumps(guarded, ensure_ascii=False))
    ...
    stage["stage"] = f"{split_name}_E10G_FULL_REMEASUREMENT"
    ...
    stage["visible_balanced_safety_action_guard"] = {
        "enabled": True,
...
def all_calls_from_summary(summary: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for key in ("dev_e10g_full_remeasurement", "validation_e10g_full_remeasurement"):
...
def apply_e10g_full_guard(summary: dict[str, Any], manifest: dict[str, Any]) -> dict[str, Any]:
    dev_stage = summary.pop("dev_e10d_full_remeasurement")
    validation_stage = summary.pop("validation_e10d_full_remeasurement")
    summary["dev_e10g_full_remeasurement"] = apply_e10g_guard_to_stage(dev_stage)
    summary["validation_e10g_full_remeasurement"] = apply_e10g_guard_to_stage(validation_stage)
    ...
    guard_rows = (
        summary["dev_e10g_full_remeasurement"]["visible_balanced_safety_action_guard"]["rows"]
        + summary["validation_e10g_full_remeasurement"]["visible_balanced_safety_action_guard"]["rows"]
    )
    status = (
        "E10G_FULL_DEV_VALIDATION_CAPTURE_PASS"
        if summary["dev_e10g_full_remeasurement"]["passed"] and summary["validation_e10g_full_remeasurement"]["passed"]
        else "E10G_FULL_DEV_VALIDATION_CAPTURE_NEEDS_REVIEW"
    )
    summary["report_version"] = "e10g-full-dev-validation-capture-v1"
    summary["status"] = status
    summary["purpose"] = "Full DEV+VALIDATION fixed parsed outputs for E10g private remeasurement"
    ...
    summary["e9_full_next_command"] = (
        ...
        "--fixed-output-file <this-e10g-full-file> "
        "--oracle-file <private-eval/expected-paths.json> "
        "--out <e10g-full-e9-summary.json> --include-rows"
    )
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    summary = apply_e10g_full_guard(summary, manifest)
...
def main() -> int:
    ...
    parser.add_argument("--manifest", type=Path, default=Path("research/experiments/e10g-full-dev-validation-remeasurement-manifest.json"))
    ...
    parser.add_argument("--dry-run", action="store_true", help="Validate E10g full remeasurement shape without external model calls")
    ...
    return 0 if args.dry_run or summary["status"] == "E10G_FULL_DEV_VALIDATION_CAPTURE_PASS" else 1
