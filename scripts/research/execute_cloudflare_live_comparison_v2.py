...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Execute the frozen ADR-020 Cloudflare packet only after a valid ADR-022 "
            "reset-window receipt has already been issued."
        )
    )
...
def main() -> None:
    ...
    evidence = CloudflareResetWindowEvidenceV1.model_validate(
        json.loads(args.evidence.read_text(encoding="utf-8"))
    )
    receipt = CloudflareResetWindowReceiptV1.model_validate(
        json.loads(args.receipt.read_text(encoding="utf-8"))
    )
    pre_live_evidence = reset_window_authorization_to_adr020_pre_live_evidence(
        receipt,
        evidence,
