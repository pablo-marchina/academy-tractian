...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Capture provider-free D02 zero-use reset-window evidence. The D01-consumed 2026-09-02 UTC window is hard-blocked.")
    ...
    parser.add_argument("--attest-exclusive-workers-ai-window-until-packet-completion", action="store_true")
...
def main() -> None:
    ...
    required = {
        ...
        "no_automated_workers_ai_consumers_since_reset": args.attest_no_automated_workers_ai_consumers_since_reset,
        "exclusive_workers_ai_window_until_packet_completion": args.attest_exclusive_workers_ai_window_until_packet_completion,
        "direct_workers_ai_route": args.attest_direct_workers_ai_route,
        "no_ai_gateway_or_prepaid_unified_billing": args.attest_no_ai_gateway_or_prepaid_unified_billing,
    }
    missing = sorted(name for name, value in required.items() if not value)
    if missing:
        raise SystemExit("all D02 attestations must be explicitly true: " + ",".join(missing))
    try:
        ensure_provider_credentials_absent()
    ...
    payload = json.dumps(evidence.model_dump(mode="json"), sort_keys=True, separators=(",", ":"), ensure_ascii=False) + "\n"
    try:
    ...
    print(json.dumps({
        "status": "D02_OPERATOR_ATTESTATION_EVIDENCE_CAPTURED_PROVIDER_FREE",
        ...
        "minimum_required_free_neurons": 9352.805376,
        "maximum_packet_neurons": 9352.805376,
