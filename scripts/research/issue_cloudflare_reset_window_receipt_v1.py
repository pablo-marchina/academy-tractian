...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Issue a Cloudflare reset-window authorization receipt without provider I/O."
    )
...
def main() -> None:
    ...
    if present:
        raise SystemExit(
            "reset-window receipt must be issued before provider secrets are provisioned: "
            + ",".join(present)
        )

    validate_frozen_reset_window_amendment()
    raw = json.loads(args.evidence.read_text(encoding="utf-8"))
    evidence = CloudflareResetWindowEvidenceV1.model_validate(raw)
    ...
    try:
        ...
        raise SystemExit("reset-window receipt output already exists") from exc
    ...
    print(
        json.dumps(
            {
                "status": "RESET_WINDOW_RECEIPT_ISSUED",
