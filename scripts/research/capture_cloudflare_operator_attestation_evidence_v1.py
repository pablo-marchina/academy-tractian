...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Capture ADR-025 operator-attested Cloudflare evidence without provider I/O or screenshot."
    )
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(
            "all ADR-025 attestations must be explicitly true: " + ",".join(missing)
        )
    ...
    try:
        ...
        raise SystemExit("operator-attestation evidence output already exists") from exc
    ...
    print(
        json.dumps(
            {
                "status": "OPERATOR_ATTESTATION_EVIDENCE_CAPTURED_PROVIDER_FREE",
                ...
                "plan_state_evidence_basis": evidence.plan_state_evidence_basis,
                "external_plan_source_artifact_required": False,
