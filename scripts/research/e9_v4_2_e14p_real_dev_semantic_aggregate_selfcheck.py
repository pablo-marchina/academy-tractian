#!/usr/bin/env python3
"""Offline pass/fail regression for the E14p E9 v4.2 aggregate gate."""
...
AGG_PATH = HERE / "e9_v4_2_e14p_real_dev_semantic_aggregate.py"
FIXTURE_PATH = HERE / "e9_v4_2_qwen36_27b_e14p_real_dev_semantic_judge_runner_selfcheck.py"

AGG_SPEC = importlib.util.spec_from_file_location("e9_v42_e14p_aggregate", AGG_PATH)
FIXTURE_SPEC = importlib.util.spec_from_file_location("e9_v42_e14p_fixture", FIXTURE_PATH)
if AGG_SPEC is None or AGG_SPEC.loader is None or FIXTURE_SPEC is None or FIXTURE_SPEC.loader is None:
    raise RuntimeError("failed to load E14p aggregate self-check dependencies")
...
def main() -> int:
    ...
    assert passed["claim_units_expected"] == 126
    assert passed["valid_unique_prediction_rows"] == 126
    assert passed["full_coverage"] is True
    assert passed["factual_claims_total"] == 0
    assert passed["factual_groundedness_rate"] == 1.0
    assert passed["underlying_model_reasoning_improvement_claimed"] is False
    assert passed["validation_gate_authorized"] is False

    assert failed["semantic_groundedness_gate_pass"] is False
    assert failed["status"] == agg.FAIL_STATUS
    assert failed["factual_claims_total"] == 1
    ...
    print(json.dumps({
        "status": "E9_V4_2_E14P_REAL_DEV_SEMANTIC_AGGREGATE_SELFCHECK_PASS",
        "full_126_row_zero_factual_fixture_passed": True,
        "zero_factual_assertions_rate_is_one": True,
