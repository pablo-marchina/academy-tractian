#!/usr/bin/env python3
"""E14j DEV-only strict JSON Schema output-enforcement candidate.

E14j inherits the complete E14f stack and the effective E14i configuration.
Relative to E14i it changes exactly one provider response-format mechanism:
historical JSON Object Mode -> JSON Schema Structured Outputs with strict=true.

The schema formalizes only the existing public E10b output contract. It does not
encode private expected paths, evaluator labels, action answers, evidence answers,
or benchmark-specific case semantics.
"""
...
e14f = load_module("e14f_parent_for_e14j", E14F_PATH)
schema = load_module("e14j_schema_for_runner", SCHEMA_PATH)

E14J_MANIFEST = Path("research/experiments/e14j-dev-only-strict-json-schema-output-manifest.json")
...
def assert_frozen_configuration(*, dry_run: bool) -> None:
    ...
    if model != REQUIRED_MODEL:
        raise AssertionError(f"E14j requires E8_GROQ_MODEL={REQUIRED_MODEL}")

    reasoning = os.getenv("E14_REASONING_EFFORT", REQUIRED_REASONING_EFFORT if dry_run else "")
    if reasoning != REQUIRED_REASONING_EFFORT:
        raise AssertionError("E14j requires E14_REASONING_EFFORT=high")
    ...
    if reasoning_format != REQUIRED_REASONING_FORMAT_ENV:
        raise AssertionError("E14j preserves E14i E14_REASONING_FORMAT=hidden")
    ...
    if response_mode != REQUIRED_RESPONSE_FORMAT_MODE:
        raise AssertionError("E14j requires E14_RESPONSE_FORMAT_MODE=json_schema_strict")
    ...
    if max_tokens != REQUIRED_MAX_COMPLETION_TOKENS:
        raise AssertionError("E14j requires unchanged E14_MAX_COMPLETION_TOKENS=1600")

    temperature = _float_env("E8_MODEL_TEMPERATURE", REQUIRED_TEMPERATURE)
    if temperature != REQUIRED_TEMPERATURE:
        raise AssertionError("E14j requires unchanged E8_MODEL_TEMPERATURE=0")

    if not dry_run and os.getenv("E8_CONFIRM_ZERO_COST") != "1":
        raise AssertionError("E14j real measurement requires E8_CONFIRM_ZERO_COST=1")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    summary["report_version"] = "e14j-dev-only-strict-json-schema-output-v1"
    summary["status"] = (
        "E14J_DEV_ONLY_STRICT_JSON_SCHEMA_OUTPUT_CAPTURE_PASS"
        if capture_pass
        else "E14J_DEV_ONLY_STRICT_JSON_SCHEMA_OUTPUT_CAPTURE_NEEDS_REVIEW"
    )
    summary["parent_e14f_capture_status"] = parent_status
    summary["e14j_structured_output_configuration"] = {
        "change_class": "provider_response_format_enforcement_only",
        "provider": "groq",
...
def main() -> int:
    ...
    parser.add_argument("--manifest", type=Path, default=E14J_MANIFEST)
    ...
    config = summary.get("e14j_structured_output_configuration", {})
    print(json.dumps({
    ...
    return 0 if summary["status"] == "E14J_DEV_ONLY_STRICT_JSON_SCHEMA_OUTPUT_CAPTURE_PASS" else 1
