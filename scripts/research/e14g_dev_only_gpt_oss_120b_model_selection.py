#!/usr/bin/env python3
"""E14g DEV-only model-selection candidate: GPT-OSS 20B -> GPT-OSS 120B only.

E14g inherits the complete E14f stack and changes exactly one variable for the
real model call: E8_GROQ_MODEL must be `openai/gpt-oss-120b`. Prompt, repair
policy, reasoning effort, temperature, completion budget, JSON Object Mode,
post-model guards, scorer and acceptance thresholds remain unchanged.

This is a model-selection experiment, not a paired causal comparison with E14f.
"""
...
e14f = load_module("e14f_parent_for_e14g", E14F_PATH)

E14G_MANIFEST = Path("research/experiments/e14g-dev-only-gpt-oss-120b-model-selection-manifest.json")
...
def assert_frozen_real_configuration(*, dry_run: bool) -> None:
    ...
    if model != REQUIRED_MODEL:
        raise AssertionError(f"E14g requires E8_GROQ_MODEL={REQUIRED_MODEL}")

    reasoning = os.getenv("E14_REASONING_EFFORT", REQUIRED_REASONING_EFFORT)
    if reasoning != REQUIRED_REASONING_EFFORT:
        raise AssertionError("E14g requires unchanged E14_REASONING_EFFORT=medium")
    ...
    if max_tokens != REQUIRED_MAX_COMPLETION_TOKENS:
        raise AssertionError("E14g requires unchanged E14_MAX_COMPLETION_TOKENS=1600")

    temperature = _float_env("E8_MODEL_TEMPERATURE", REQUIRED_TEMPERATURE)
    if temperature != REQUIRED_TEMPERATURE:
        raise AssertionError("E14g requires unchanged E8_MODEL_TEMPERATURE=0")

    if not dry_run and os.getenv("E8_CONFIRM_ZERO_COST") != "1":
        raise AssertionError("E14g real measurement requires E8_CONFIRM_ZERO_COST=1")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    summary["report_version"] = "e14g-dev-only-gpt-oss-120b-model-selection-v1"
    summary["status"] = (
        "E14G_DEV_ONLY_GPT_OSS_120B_MODEL_SELECTION_CAPTURE_PASS"
        if capture_pass
        else "E14G_DEV_ONLY_GPT_OSS_120B_MODEL_SELECTION_CAPTURE_NEEDS_REVIEW"
    )
    summary["parent_e14f_capture_status"] = parent_status
    summary["e14g_model_selection"] = {
        "change_class": "model_selection_only",
        "provider": "groq",
        "model": REQUIRED_MODEL,
        "parent_model": "openai/gpt-oss-20b",
        ...
        "causal_interpretation": "absolute model-selection gate only; separate generations are not paired causal observations",
    }
...
def main() -> int:
    ...
    parser.add_argument("--manifest", type=Path, default=E14G_MANIFEST)
    ...
    model_meta = summary.get("e14g_model_selection", {})
    print(json.dumps({
    ...
    return 0 if summary["status"] == "E14G_DEV_ONLY_GPT_OSS_120B_MODEL_SELECTION_CAPTURE_PASS" else 1
