#!/usr/bin/env python3
"""E14p-specific real DEV semantic judge runner for E9 v4.2.

Reuses the exact reliability-qualified Qwen judge implementation, frozen system
prompt, provider settings, JSON contract, and per-call request partition. The
only packet-shape change is the aggregate shape observed before any E14p labels:
126 claim units with source counts 24/78/6/18.

A distinct E14p attempt lock prevents collision with consumed historical/E14o
measurements. No private oracle, scorer rows, VALIDATION, or LOCKED_TEST are read.
"""
...
PARENT_PATH = HERE / "e9_v4_2_qwen36_27b_e14o_real_dev_semantic_judge_runner.py"
SPEC = importlib.util.spec_from_file_location("e9_v42_e14o_parent_for_e14p", PARENT_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("failed to load frozen E14o semantic judge runner")
...
RESULT_VERSION = "e9-v4.2-qwen36-27b-e14p-real-dev-semantic-judge-v1"
LOCK_VERSION = "e9-v4.2-e14p-real-dev-semantic-attempt-lock-v1"
PASS_STATUS = "E9_V4_2_QWEN_E14P_REAL_DEV_SEMANTIC_JUDGE_CAPTURE_PASS"
OPERATIONAL_FAILURE_STATUS = "E9_V4_2_QWEN_E14P_REAL_DEV_OPERATIONAL_FAILURE"
CONTRACT_FAILURE_STATUS = "E9_V4_2_QWEN_E14P_REAL_DEV_OUTPUT_CONTRACT_FAILURE"
CANDIDATE = "E14p-after-fixed-E14o-and-E14n-v1.1"
...
def validate_packet(packet: Any) -> list[dict[str, Any]]:
    ...
    if dict(source_counts) != EXPECTED_SOURCE_COUNTS:
        raise AssertionError("claim packet source-field counts do not match preregistered E14p packet")
    return calls
...
def consume_real_measurement_attempt(out: Path) -> Path:
    ...
    if out.exists():
        raise SystemExit("E14p real DEV judge-result output already exists; rerun is forbidden")
    if lock.exists():
        raise SystemExit("E14p real DEV semantic measurement attempt already consumed; rerun requires an explicit amendment")
    lock.parent.mkdir(parents=True, exist_ok=True)
    lock.write_text(json.dumps({
        "report_version": LOCK_VERSION,
        "status": "E14P_REAL_DEV_SEMANTIC_MEASUREMENT_ATTEMPT_CONSUMED",
        "judge_model": MODEL,
...
def run(args: argparse.Namespace) -> tuple[dict[str, Any], int]:
    ...
    if code == 0:
        ...
        local_results["candidate"] = CANDIDATE
