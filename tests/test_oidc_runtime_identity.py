...
def test_oidc_provider_verifies_asymmetric_jwt_and_maps_tenant_context(rsa_keys) -> None:
    ...
    assert "unknown:permission" not in body["permissions"]
...
def test_oidc_provider_rejects_wrong_audience_missing_org_and_wrong_authorized_party(rsa_keys) -> None:
    private, public = rsa_keys
    client = _client(_provider(public))
    assert client.get("/context", headers={"Authorization": f"Bearer {_token(private, aud='other-api')}"}).status_code == 401
    assert client.get("/context", headers={"Authorization": f"Bearer {_token(private, organization_id=None)}"}).status_code == 401
    assert client.get("/context", headers={"Authorization": f"Bearer {_token(private, azp='https://evil.example.com')}"}).status_code == 401


def test_oidc_provider_enforces_required_claims_and_ttl(rsa_keys) -> None:
    private, public = rsa_keys
    ...
    assert client.get("/context", headers={"Authorization": f"Bearer {_token(private)}"}).status_code == 200
    assert client.get("/context", headers={"Authorization": f"Bearer {_token(private, role=None)}"}).status_code == 401
    now = int(time())
    assert client.get("/context", headers={"Authorization": f"Bearer {_token(private, iat=now-10, exp=now+7200)}"}).status_code == 401


def test_oidc_provider_rejects_symmetric_algorithm_and_privilege_without_second_gate(rsa_keys) -> None:
    _, public = rsa_keys
