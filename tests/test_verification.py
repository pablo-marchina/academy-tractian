...
STRUCTURAL_CHECKS = (
    "trace_lifecycle", "production_trace_identity", "proposal_contract_validity",
    "identity_seed_model_isolation", "execution_chain_integrity", "policy_denial_containment",
    "provider_free_trace", "model_call_provenance", "read_only_action_safety", "terminal_consistency",
)


def _evaluation(*, failed: set[str] | None = None) -> list[dict[str, object]]:
    failed = failed or set()
    return [{"check_name": name, "passed": name not in failed, "blocking": True} for name in STRUCTURAL_CHECKS]


def _run(**overrides: object) -> dict[str, object]:
    payload: dict[str, object] = {"run_id": "run_test", "completed": True, "terminal_reason_code": None, "terminal_response_mode": "complete", "tool_calls": 2}
...
def test_structural_green_is_not_promoted_to_overall_quality() -> None:
    report = verify_persisted_run(run=_run(), events=_clean_events(), evaluation=_evaluation())
...
def test_safe_failure_cannot_receive_overall_green_when_structural_checks_pass() -> None:
    report = verify_persisted_run(run=_run(terminal_reason_code="DECISION_SOURCE_FAILURE", terminal_response_mode="unavailable"), events=_clean_events(), evaluation=_evaluation())
    dimensions = report.by_name()
    assert dimensions["runtime_integrity"].status == "VERIFIED"
...
def test_repeated_401_loop_is_detected_as_non_progress() -> None:
    ...
    report = verify_persisted_run(run=_run(terminal_reason_code="TOOL_CALL_BUDGET_EXHAUSTED", terminal_response_mode="unavailable", tool_calls=6), events=events, evaluation=_evaluation())
    dimensions = report.by_name()
...
def test_structural_failure_remains_a_hard_failure() -> None:
    report = verify_persisted_run(run=_run(), events=_clean_events(), evaluation=_evaluation(failed={"execution_chain_integrity"}))
...
def test_independent_oracles_can_verify_functional_and_evidence_dimensions() -> None:
    report = verify_persisted_run(
        run=_run(), events=_clean_events(), evaluation=_evaluation(),
        oracle_results={
            "functional_success": IndependentOracleResult(status="VERIFIED", source="golden-functional-oracle-v1", summary="All mandatory functional requirements were observed.", evidence=("case:G01",)),
            "evidence_sufficiency": IndependentOracleResult(status="VERIFIED", source="evidence-requirement-oracle-v1", summary="All required evidence classes were observed before terminal.", evidence=("condition_evidence",)),
        },
    )
...
def test_nonblocking_semantic_unknown_does_not_falsely_fail_verified_hard_gates() -> None:
    report = verify_persisted_run(
        run=_run(), events=_clean_events(), evaluation=_evaluation(),
        oracle_results={
            "functional_success": IndependentOracleResult(status="VERIFIED", source="functional-oracle-v1", summary="Functional rubric passed."),
            "evidence_sufficiency": IndependentOracleResult(status="VERIFIED", source="evidence-oracle-v1", summary="Evidence rubric passed."),
        },
    )
    assert report.overall_status == "VERIFIED"
