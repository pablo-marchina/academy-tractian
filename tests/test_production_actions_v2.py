...
def _principal(*, permissions: frozenset[Permission] | None = None, company: str = "company-1") -> ProductionActionPrincipal:
    return ProductionActionPrincipal(
        ...
        resource_company_bindings=(ResourceCompanyBinding(resource_id="analysis-1", company_id=company),),
    )
...
def _resolver(principal: ProductionActionPrincipal):
    def resolve(*, user_id: str) -> ProductionActionPrincipal:
        if user_id != principal.user_id:
            return principal.model_copy(update={"user_id": user_id})
        return principal
    return resolve
...
def _proposal_runtime(tmp_path, *, principal: ProductionActionPrincipal, transport: FakeTransport, custody: PendingActionCustody):
    ...
    source = ScriptedSource(
        ...
        ControllerDecision(
            ...
            final={"decision": "ORIENT", "response_mode": "partial", "message": "Action requires explicit operator confirmation."},
        ),
    )
...
def _request(request_id: str = "req-action-v2-1") -> ProductionRequest:
...
def test_valid_action_proposal_is_custodied_but_never_executes_transport(tmp_path) -> None:
    ...
    runtime, store = _proposal_runtime(tmp_path, principal=_principal(), transport=transport, custody=custody)
    trace = runtime.run(_request())

    assert transport.calls == []
    origin_run_id = str(store.list_runs(limit=10)[0]["run_id"])
    pending = custody.list_safe_for_origin(origin_run_id)
...
def test_permission_or_scope_failure_creates_no_pending_action(tmp_path) -> None:
    ...
    for suffix, principal, expected in cases:
        root = tmp_path / suffix
        custody = PendingActionCustody(root / "private-actions.duckdb")
        runtime, store = _proposal_runtime(root, principal=principal, transport=FakeTransport(), custody=custody)
        trace = runtime.run(_request(f"req-{suffix}"))
        origin_run_id = str(store.list_runs(limit=10)[0]["run_id"])
        assert custody.list_safe_for_origin(origin_run_id) == []
        check = next(event for event in trace.events if event.event_type == "policy_check")
        assert check.metadata["violation"] == expected


def test_confirmed_action_executes_exact_custodied_action_once_and_persists_claim(tmp_path) -> None:
    custody = PendingActionCustody(tmp_path / "private-actions.duckdb")
    runtime, store = _proposal_runtime(tmp_path, principal=_principal(), transport=FakeTransport(), custody=custody)
    ...
    key_hash = sha256(private.idempotency_key.encode("utf-8")).hexdigest()
...
def test_ambiguous_transport_failure_keeps_claim_after_restart_and_forbids_retry(tmp_path) -> None:
    custody = PendingActionCustody(tmp_path / "private-actions.duckdb")
    runtime, store = _proposal_runtime(tmp_path, principal=_principal(), transport=FakeTransport(), custody=custody)
    runtime.run(_request())
    pending = custody.list_safe_for_origin(str(store.list_runs(limit=10)[0]["run_id"]))[0]
    ...
    private = custody.get_private_for_requester(action_id=pending.action_id, requester_user_id="user-1")
    key_hash = sha256(private.idempotency_key.encode("utf-8")).hexdigest()
    restarted = DuckDBActionIdempotencyLedger(ledger_path)
    assert restarted.get(key_hash)["state"] == "UNCERTAIN"  # type: ignore[index]
...
def test_wrong_requester_cannot_read_private_pending_payload(tmp_path) -> None:
    custody = PendingActionCustody(tmp_path / "private-actions.duckdb")
    runtime, store = _proposal_runtime(tmp_path, principal=_principal(), transport=FakeTransport(), custody=custody)
    runtime.run(_request())
    pending = custody.list_safe_for_origin(str(store.list_runs(limit=10)[0]["run_id"]))[0]
    with pytest.raises(PermissionError, match="action_requester_mismatch"):
        custody.get_private_for_requester(action_id=pending.action_id, requester_user_id="user-2")
