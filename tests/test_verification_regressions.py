...
from academy_tractian.functional_oracle import EvidenceRequirementSpec, RequiredToolGroup, ToolResourceRequirement, evaluate_evidence_trace
from academy_tractian.verification import verify_persisted_run


def _structural_green() -> list[dict[str, object]]:
    return [{"check_name": name, "passed": True, "blocking": True} for name in (
...
def test_success_for_same_tool_on_other_asset_cannot_rehabilitate_failed_evidence() -> None:
    ...
    trace = RunTrace(run_id="run_correlation", scenario_id="prod:correlation", config_hash="a" * 64, identity_binding_id="identity", seed_ref="none", events=events)
