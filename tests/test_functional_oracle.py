...
from academy_tractian.functional_oracle import (
    EvidenceRequirementSpec, FunctionalCaseSpec, RequiredToolGroup, ToolResourceRequirement,
    evaluate_evidence_trace, evaluate_functional_trace,
)
...
def _trace(events: list[TraceEvent]) -> RunTrace:
    return RunTrace(run_id="run_fixture", scenario_id="prod:fixture", config_hash="a" * 64, identity_binding_id="identity_fixture", seed_ref="none", events=events)
...
def _successful_bilateral_trace() -> RunTrace:
    return _trace([
        ...
        _event(9, "final_response", result={"decision": "ORIENT", "response_mode": "complete", "reason_code": None, "message": "A requires attention before B."}),
...
def _bilateral_functional_spec() -> FunctionalCaseSpec:
    return FunctionalCaseSpec(
        ...
        allowed_response_modes=(ResponseMode.COMPLETE, ResponseMode.PARTIAL), minimum_tool_calls=4,
    )
...
def test_independent_functional_oracle_accepts_multiple_valid_condition_tools() -> None:
    ...
    assert report.status == "VERIFIED" and report.failures == () and report.tool_calls == 4
...
def test_independent_functional_oracle_rejects_safe_runtime_failure() -> None:
    trace = _trace([
        _event(0, "run_started"),
        _event(1, "final_response", result={"decision": "ABSTAIN", "response_mode": "unavailable", "reason_code": "TOOL_CALL_BUDGET_EXHAUSTED", "message": "Budget exhausted."}),
...
def test_evidence_oracle_requires_successful_condition_evidence_for_each_asset() -> None:
    ...
    assert evaluate_evidence_trace(_successful_bilateral_trace(), spec).status == "VERIFIED"
