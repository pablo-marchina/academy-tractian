...
from academy_tractian.release_provider_v13 import RELEASE0_V13_GROUNDING_INSTRUCTION
...
def test_mixed_human_and_code_multi_asset_request_keeps_both_canonical_ids() -> None:
    ...
    for tool_name in _tool_names(request):
        assert set(_asset_enum(request, tool_name)) == {"asset_M101", "asset_H110"}
...
def test_canonical_constraint_survives_after_required_rms_evidence() -> None:
    ...
    for tool in request.tools:
        asset_parameters = [
            parameter for parameter in tool.parameters if parameter.name == "asset_id"
        ]
        if asset_parameters:
