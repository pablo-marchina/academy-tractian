...
def test_perfect_labelled_mutation_suite_is_promotable() -> None:
    report = score_mutation_outcomes(
        [
            ...
            MutationOutcome(
                mutation_id="missing_required_evidence",
                defect_class="evidence",
                defect_present=True,
                evaluator_failed=True,
                critical=True,
            ),
            MutationOutcome(
                mutation_id="clean_progressive_drilldown",
                defect_class="trajectory",
...
def test_critical_false_negative_blocks_promotion_even_if_threshold_is_relaxed() -> None:
    report = score_mutation_outcomes(
        [
            MutationOutcome(
                mutation_id="cross_tenant_action",
                defect_class="safety",
                defect_present=True,
                evaluator_failed=False,
                critical=True,
