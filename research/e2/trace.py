...
def validate_trace(trace: RunTrace) -> list[str]:
    ...
    if [e.sequence for e in trace.events] != list(range(len(trace.events))):
        errors.append("event sequence must be contiguous from zero")
    terminal_positions = [i for i, e in enumerate(trace.events) if e.event_type in TERMINAL_EVENTS]
