...
from research.e2.models import AgentCase, BoundContext, ConclusionOracle, DecisionOracle, EnvironmentSpec, EvaluationSpec, ExecutionBinding, EvidenceOracle, Provenance, Scenario, ScenarioInput, PolicyOracle, RunTrace, TraceEvent, TrajectoryOracle, ToolSpec
...
def scenario() -> Scenario:
    return Scenario(
        scenario_id="CEN-01", title="test", ticket_ids=["TKT-INV-04"], split_group_id="asset_G501",
        ...
        environment=EnvironmentSpec(), decision_oracle=DecisionOracle(), policy_oracle=PolicyOracle(), evidence_oracle=EvidenceOracle(),
        conclusion_oracle=ConclusionOracle(source_resolution_text="x"), trajectory_oracle=TrajectoryOracle(), evaluation=EvaluationSpec(p1_success_source="x"),
    )
...
def test_replay_is_deterministic():
    store = ReplayStore(); request = {"method": "GET", "path": "/x", "query": {"seed": "a"}}
    store.record(request, {"mode": "partial"}); assert store.replay(request) == {"mode": "partial"}
    with pytest.raises(ValueError): store.record(request, {"mode": "complete"})
...
def test_registry_is_complete():
    ...
    validate_registry(); assert len(TOOLS) == 18; assert sum(t.kind.value == "action" for t in TOOLS) == 5
...
def test_strict_action_validation_catches_runtime_permissiveness():
    from research.e2.tool_registry import get_tool
    from research.e2.validation import validate_arguments
    issues = validate_arguments(get_tool("update_asset_config"), {"asset_id": "asset_V301", "body": {"changes": {"criticality": "banana"}, "justification": "valid enough justification"}})
    assert any(issue.code == "INVALID_ENUM" for issue in issues)


def test_resource_policy_blocks_cross_company_action():
    from research.e2.policy import ResourcePolicy
    from research.e2.tool_registry import get_tool
    from research.e2.models import Permission
    decision = ResourcePolicy(user_permissions={Permission.ACTION_HIGH}, user_company_id="comp_a", resource_company_lookup={"asset_b": "comp_b"}).check(get_tool("update_asset_config"), {"asset_id": "asset_b", "body": {"justification": "justification long enough"}})
    assert not decision.allowed and decision.code == "RESOURCE_SCOPE_DENIED"
